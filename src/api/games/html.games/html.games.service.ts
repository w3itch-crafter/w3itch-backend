...
export class HtmlGamesService implements ISpecificGamesService {
    ...
    public async uploadGame(user: Pick<UserJWTPayload, 'username' | 'id'>, file: Express.Multer.File, game: CreateGameProjectDto | Game): Promise<void> {
        const { charset } = game;
    ...
    async extractGame(user: Pick<UserJWTPayload, "username" | "id">, zip: AdmZip, game: CreateGameProjectDto | Game, entryPath: string) {
        const { gameName } = game;
